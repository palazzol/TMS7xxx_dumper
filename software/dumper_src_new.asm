...
; (This code has not been re-assembled, yet)
;
; Dumper code for:
...
START:
        MOVP    %>FF, P6    ; PORTB = FF, PORTB.0=1 (MC high, make sure external ROM enabled)
        MOVP    %>0C, B     ; B  = 0C
...
CPYLOOP:
        ...
        JNZ     LOOP        ; if we are not done, loop to copy next byte
...
RAMCODE:
        MOVP    %>FE,P6     ; PORTB = FE, PORTB.0=0 (Flip the MC pin low, internal ROM enabled)
        MOV     %>FF,B      ; B = FF
DELAY:
        DEC     B           ; B = B-1
